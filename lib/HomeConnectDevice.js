...
class HomeConnectDevice extends homey_oauth2app_1.OAuth2Device {
    ...
    async onOAuth2Init() {
        ...
        this.haId = haId;
        this.oAuth2Client.on(`${haId}:status`, this.handleEvent.bind(this));
        this.oAuth2Client.on(`${haId}:notify`, this.handleEvent.bind(this));
        this.oAuth2Client.on(`${haId}:event`, this.handleEvent.bind(this));
        this.oAuth2Client.on(`${haId}:connected`, () => {
            if (DEBUG)
        ...
        this.oAuth2Client.on(`${haId}:disconnected`, () => {
            this.log('Device disconnected');
            this.setUnavailable()
                .catch(this.error);
        });
        this.oAuth2Client.on(`${haId}:fatal-error`, () => {
            this.setUnavailable('Token error; please repair the device and authenticate again.')
                .catch(this.error);
        });
    ...
    async sync() {
        ...
        try {
            ...
            try {
                ...
                this.error("Sync commandsa failed", e);
            }
            await this.onSync();

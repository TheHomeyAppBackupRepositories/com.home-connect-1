...
class CoolingDeviceBase extends HomeConnectDevice_1.default {
    ...
    async setRefrigeratorImages() {
        ...
        await this.createAndSetImageFromAPI(images, 'Refrigeration.FridgeFreezer.Status.DoorCameraPresentRefrigerator', 'fridgeDoor', `${this.homey.__('fridge')} - ${this.homey.__('door')}`);
        await this.createAndSetImageFromAPI(images, 'Refrigeration.FridgeFreezer.Status.InteriorCameraPresentRefrigerator', 'fridgeInterior', `${this.homey.__('fridge')} - ${this.homey.__('interior')}`);
    }
